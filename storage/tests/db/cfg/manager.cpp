...
TEST_F(cfg_manager_test, ConcurrentRead) {
  ...
  for (int i = 0; i < num_threads; ++i) {
    threads.emplace_back([&cfg_file, &successful_reads, &print_mutex]() {
      try {
        manager reader_mgr;
        reader_mgr.connect(cfg_file.string());
        // Создаём базовую конфигурацию для каждого читателя
        reader_mgr.create_db();
