...
bool user_autorization::validate_user(const helpers::handler_context& context,
  ...
  if constexpr (!user_whitelist.empty()) {
    if (std::none_of(user_whitelist.cbegin(),
                     user_whitelist.cend(),
                     [&current_user](const auto& user) {
...
template <>
bool user_autorization::validate_user_command<command_type::start>(
...
template <>
bool user_autorization::validate_user_command<command_type::user_token_add>(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE
  if (!validate_user(context, message)) {
    return false;
...
template <>
bool user_autorization::validate_user_command<command_type::user_token_erase>(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE
  if (!validate_user(context, message)) {
    return false;
...
template <>
bool user_autorization::validate_user_command<command_type::user_view>(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE
  if (!validate_user(context, message)) {
    return false;
...
template <>
bool user_autorization::validate_user_command<command_type::group_list>(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE
  if (!validate_user(context, message)) {
    return false;
...
template <>
bool user_autorization::validate_user_command<command_type::group_create>(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE
  if (!validate_user(context, message)) {
    return false;
...
template <>
bool user_autorization::validate_user_command<command_type::group_delete>(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE
  if (!validate_user(context, message)) {
    return false;
...
template <>
bool user_autorization::validate_user_command<command_type::group_user_add>(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE
  if (!validate_user(context, message)) {
    return false;
...
template <>
bool user_autorization::validate_user_command<command_type::group_leave>(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE
  if (!validate_user(context, message)) {
    return false;
...
bool user_autorization::validate_user_command(const helpers::handler_context& context,
                                              TgBot::Message::Ptr message) {
  AUTOMEASURE
