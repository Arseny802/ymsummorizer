...
void group::init_commands(const helpers::handler_context& context) {
  ...
  context.bot.getEvents().onCommand("group_list",
                                    std::bind(&group::on_group_list, this, context, std::placeholders::_1));
  context.bot.getEvents().onCommand("group_create",
                                    std::bind(&group::on_group_create, this, context, std::placeholders::_1));
  context.bot.getEvents().onCommand("group_delete",
...
void group::on_group_list(const helpers::handler_context& context, TgBot::Message::Ptr message) {
  ...
  ymsummorizer_callback_result::ptr result = context.cmd_cache.callback_commands[command_type::group_list](ui);
  if (result->ok) {
    ...
    for (auto& group: group_list->groups) {
      ...
      for (auto& user_id: group.user_ids) {
        ...
        ss << " - _" << common::user::hide_special_chars(user_it->second.name) << "_ (https://t.me/"
           << common::user::hide_special_chars(user_it->second.login_tg) << ")"
           << "\n";
      }
      ss << "\n\n";
    }
...
void group::on_group_create(const helpers::handler_context& context, TgBot::Message::Ptr message) {
  ...
  context.cmd_cache.command_queue_emplace_next(message->from->username, [this, &context](TgBot::Message::Ptr message) {
    ...
    ymsummorizer_callback_result::ptr result = context.cmd_cache.callback_commands[command_type::group_create](ui);
    if (result->ok) {
      context.bot.getApi().sendMessage(message->chat->id, fmt::format("Группа '{}' создана.", message->text));
    } else {
...
void group::on_group_delete(const helpers::handler_context& context, TgBot::Message::Ptr message) {
  ...
  context.cmd_cache.command_queue_emplace_next(message->from->username, [this, &context](TgBot::Message::Ptr message) {
    ...
    ui.timestamp = message->date;
    ui.arguments[user_interaction::key_group_name] = message->text;
    ymsummorizer_callback_result::ptr result = context.cmd_cache.callback_commands[command_type::group_delete](ui);
    if (result->ok) {
      context.bot.getApi().sendMessage(message->chat->id, fmt::format("Группа '{}' удалена.", message->text));
    } else {
      context.bot.getApi().sendMessage(message->chat->id,
