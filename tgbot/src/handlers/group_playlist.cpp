...
void group_playlist::init_commands(const helpers::handler_context& context) {
  ...
  context.bot.getEvents().onCommand(
      "group_playslit_list", std::bind(&group_playlist::on_group_playslit_list,
                                       this, context, std::placeholders::_1));
  context.bot.getEvents().onCommand(
      "group_playslit_view", std::bind(&group_playlist::on_group_playslit_view,
                                       this, context, std::placeholders::_1));
  context.bot.getEvents().onCommand(
      "group_playslit_add", std::bind(&group_playlist::on_group_playslit_add,
                                      this, context, std::placeholders::_1));
  context.bot.getEvents().onCommand(
      "group_playslit_remove",
      std::bind(&group_playlist::on_group_playslit_remove, this, context,
                std::placeholders::_1));
}

void group_playlist::on_group_playslit_list(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE_TG
...
void group_playlist::on_group_playslit_view(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE_TG
  if (!validate_user_command<command_type::group_user_remove>(context,
                                                              message)) {
...
void group_playlist::on_group_playslit_add(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE_TG
  if (!validate_user_command<command_type::group_playslit_add>(context,
                                                               message)) {
    return;
  }
  ...
  user_interaction::ptr prolonged_interaction =
      std::make_shared<user_interaction>();
  ...
  context.cmd_cache.command_queue_emplace_next(
      ...
      [this, prolonged_interaction, &context](TgBot::Message::Ptr message) {
        const auto group_name = message->text;
        if (!common::group::is_valid_name(group_name)) {
          context.bot.getApi().sendMessage(
              message->chat->id,
              "Название группы не подходит :(\n Попробуйте ещё раз вызвать "
              "команду с новым названием.");
          return false;
        }

        log()->info(
            "group_playslit_add - stage 1. User '{}' passed group_name '{}'.",
            message->from->username, group_name);
  ...
  context.cmd_cache.command_queue_emplace_next(
      ...
      [this, prolonged_interaction, &context](TgBot::Message::Ptr message) {
        ...
        log()->info(
            "group_playslit_add - stage 2. User '{}' passed playlist_name "
            "'{}'.",
            message->from->username, playlist_name);
        prolonged_interaction->arguments[user_interaction::key_playlist_name] =
            std::move(playlist_name);

        ymsummorizer_callback_result::ptr result =
            context.cmd_cache
                .callback_commands[command_type::group_playslit_add](
        ...
        if (result->ok) {
          context.bot.getApi().sendMessage(
              ...
              fmt::format("Плейлист '{}' добавлен в группу '{}'.",
                          prolonged_interaction
                              ->arguments[user_interaction::key_playlist_name],
                          prolonged_interaction
                              ->arguments[user_interaction::key_group_name]));
        } else {
          context.bot.getApi().sendMessage(
              message->chat->id,
...
void group_playlist::on_group_playslit_remove(
    const helpers::handler_context& context, TgBot::Message::Ptr message) {
  AUTOMEASURE_TG
  if (!validate_user_command<command_type::group_user_remove>(context,
                                                              message)) {
