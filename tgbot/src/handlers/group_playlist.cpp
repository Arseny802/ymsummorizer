...
void group_playlist::init_commands(const helpers::handler_context& context) {
  ...
  context.bot.getEvents().onCommand(
      "group_playslit_add",
      std::bind(&group_playlist::on_group_playslit_add, this, context, std::placeholders::_1));
  context.bot.getEvents().onCommand(
      "group_playslit_remove",
      std::bind(&group_playlist::on_group_playslit_remove, this, context, std::placeholders::_1));
}

void group_playlist::on_group_playslit_list(const helpers::handler_context& context,
...
void group_playlist::on_group_playslit_view(const helpers::handler_context& context,
                                            TgBot::Message::Ptr message) {
  AUTOMEASURE
  if (!validate_user_command<command_type::group_user_remove>(context, message)) {
    return;
...
void group_playlist::on_group_playslit_add(const helpers::handler_context& context,
                                           TgBot::Message::Ptr message) {
  AUTOMEASURE
  ...
  context.cmd_cache.command_queue_emplace_next(
      ...
      [this, prolonged_interaction, &context](TgBot::Message::Ptr message) {
        ...
        log()->info("group_playslit_add - stage 1. User '{}' passed group_name '{}'.",
                    message->from->username,
                    group_name);
  ...
  context.cmd_cache.command_queue_emplace_next(
      ...
      [this, prolonged_interaction, &context](TgBot::Message::Ptr message) {
        ...
        ymsummorizer_callback_result::ptr result =
            context.cmd_cache.callback_commands[command_type::group_playslit_add](
...
void group_playlist::on_group_playslit_remove(const helpers::handler_context& context,
                                              TgBot::Message::Ptr message) {
  AUTOMEASURE
  if (!validate_user_command<command_type::group_user_remove>(context, message)) {
    return;
