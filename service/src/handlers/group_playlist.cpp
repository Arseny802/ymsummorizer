...
tgbot::ymsummorizer_callback_result::ptr group_playlist::on_group_playslit_list(
    const tgbot::user_interaction& ui) {
  AUTOTRACE
...
tgbot::ymsummorizer_callback_result::ptr group_playlist::on_group_playslit_view(
    const tgbot::user_interaction& ui) {
  AUTOTRACE
  auto ret = std::make_shared<tgbot::ymsummorizer_callback_result>();
  ret->ok = true;
...
tgbot::ymsummorizer_callback_result::ptr group_playlist::on_group_playslit_add(
  ...
  const auto found_playlist_name_iter =
      interaction.arguments.find(tgbot::user_interaction::key_playlist_name);
  ...
  try {
    ...
    auto it = std::find_if(groups.begin(), groups.end(), [&group_name](const common::group& g) {
    ...
    for (const auto& user_id : it->user_ids) {
      const auto user_it =
          std::find_if(stored_users.cbegin(),
                       stored_users.cend(),
      ...
      try {
        ...
        if (!playlist_ym) {
          log()->error("Failed to create playlist '{}' for user '{}'.",
                       playlist.name,
                       user_it->login_yandex);
          continue;
        }
...
tgbot::ymsummorizer_callback_result::ptr group_playlist::on_group_playslit_remove(
    const tgbot::user_interaction& ui) {
  AUTOTRACE
  auto ret = std::make_shared<tgbot::ymsummorizer_callback_result>();
  ret->ok = true;
